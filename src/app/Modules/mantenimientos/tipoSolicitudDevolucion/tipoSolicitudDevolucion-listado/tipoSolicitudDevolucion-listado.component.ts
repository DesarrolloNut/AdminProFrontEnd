...
import { SolicitudDevolucion } from 'src/app/Modules/inventario/solicitud-devolucion/models/SolicitudDevolucion';
import { DataApi } from 'src/app/shared/enums/DataApi.enum';
import Swal from 'sweetalert2';
import { TipoSolicitudDevolucionViewModel } from '../models/ModuloViewModel';
...
export class TipoSolicitudDevolucionListadoComponent implements OnInit {
  ...
  info:TipoSolicitudDevolucionViewModel[]=[
    { id: 1, nombre: "Factura", moduloId:11,modulo:"INVENTARIOSOLICITUDDEVOLUCION", companiaId: 1 },
    { id: 2, nombre: "Cambio", moduloId:11, modulo:"INVENTARIOSOLICITUDDEVOLUCION", companiaId: 1 },
    { id: 3, nombre: "Averias", moduloId:11, modulo:"INVENTARIOSOLICITUDDEVOLUCION", companiaId: 1 },
    { id: 4, nombre: "Averias", moduloId:12, modulo:"INVENTARIOSOLICITUDDEVOLUCION", companiaId: 1 },
  ]
  ...
  confirmarEliminacionTipoSolicitudDevolucion(id:number){
    Swal.fire({
      title: 'Estas seguro que quieres eliminar este Tipo Solicitud Devolución?',
    ...
    this.httpService.DoPostAny<any>(DataApi.TipoSolicitudDevolucion,
      "Delete", parametros).subscribe(response => {
        if (!response.ok) {
         ...
         Swal.fire(
           'Confirmado',
           'Se ha eliminado el tipo solicitud devolución correctamente.',
           'success'
         )
          this.getData();
        }
